...
class FakeSSH:
    ...
    async def execute(self, command: str) -> tuple[str, str, int]:
        ...
        responses: dict[str, tuple[str, str, int]] = {
            ...
            "cat /proc/loadavg": ("0.25 0.10 0.05 1/100 10", "", 0),
            "ubus call network.wireless status": (wireless, "", 0),
            "cat /tmp/dhcp.leases": (
            ...
            "ip route show": ("default via 192.0.2.1 dev eth0", "", 0),
            "ping -c 2 -W 1 192.0.2.1": ("ok", "", 0),
            ...
            "nslookup example.test 192.0.2.53": (
                "Name: example.test\nAddress: 192.0.2.4",
                "",
                0,
            ),
            "iwinfo wlan0 scan": (
                "Cell 01 - Address: 02:00:00:00:00:02\n"
                '  ESSID: "Neighbor"\n'
...
async def test_system_wifi_dhcp_and_firewall_parsing(
...
async def test_config_package_log_and_dhcp_workflows(
    explorer: OpenWRTExplorer,
) -> None:
...
async def test_historical_diagnostics_and_context_contracts(
    ...
    assert context["dhcp_leases_count"] == 1


async def test_network_tool_dtos_are_backward_compatible(
    explorer: OpenWRTExplorer,
) -> None:
...
async def test_invalid_inputs_fail_before_ssh(
    explorer: OpenWRTExplorer,
    call: Any,
) -> None:
    ssh = explorer.ssh
