...
class SSHConnection:
    ...
    async def connect(self) -> bool:
        ...
        async with self._lock:
            ...
            try:
                ...
                logging.info(
                    f"[openwrt] SSH connection established: {OPENWRT_USER}@{OPENWRT_HOST}"
                )
                return True
    ...
    async def execute(self, command: str) -> Tuple[str, str, int]:
        ...
        try:
            ...
            logging.warning(
                f"[openwrt] SSH connection lost ({e}), attempting reconnect..."
            )
...
class OpenWRTExplorer:
    ...
    async def get_system_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_used_percent": round((1 - mem_free / mem_total) * 100, 1)
            if mem_total
            else 0,
        }
                    ...
                    interfaces.append(
                        {
                            ...
                            "ifname": iface.get(
                                "ifname", iface.get("section", "unknown")
                            ),
    ...
    async def get_router_logs(
        self, lines: int = 50, filter_level: str = "all"
    ) -> Dict[str, Any]:
        """Fetch router logs."""
    ...
    async def search_router_logs(
        self, search_term: str, max_results: int = 30
    ) -> Dict[str, Any]:
    ...
    async def search_dhcp_logs(
        self, search_term: str, hours_back: int = 24
    ) -> Dict[str, Any]:
...
def register_openwrt_tools(mcp):
    ...
    async def test_router_connection() -> str:
        ...
        return json.dumps(
            await get_explorer().test_connection(), indent=2, ensure_ascii=False
        )
    ...
    async def get_router_info() -> str:
        ...
        return json.dumps(
            await get_explorer().get_system_info(), indent=2, ensure_ascii=False
        )
    ...
    async def get_router_wifi_status() -> str:
        ...
        return json.dumps(
            await get_explorer().get_wifi_status(), indent=2, ensure_ascii=False
        )
    ...
    async def get_router_dhcp_leases() -> str:
        ...
        return json.dumps(
            await get_explorer().list_dhcp_leases(), indent=2, ensure_ascii=False
        )
    ...
    async def get_router_firewall_rules() -> str:
        ...
        return json.dumps(
            await get_explorer().get_firewall_rules(), indent=2, ensure_ascii=False
        )
    ...
    async def read_router_uci_config(config_name: str) -> str:
        ...
        return json.dumps(
            await get_explorer().read_uci_config(config_name),
            indent=2,
            ensure_ascii=False,
        )
    ...
    async def list_router_packages() -> str:
        ...
        return json.dumps(
            await get_explorer().list_installed_packages(), indent=2, ensure_ascii=False
        )
    ...
    async def get_router_logs(lines: int = 50, filter_level: str = "all") -> str:
        ...
        return json.dumps(
            await get_explorer().get_router_logs(lines, filter_level),
            indent=2,
            ensure_ascii=False,
        )
    ...
    async def search_router_logs(search_term: str, max_results: int = 30) -> str:
        ...
        return json.dumps(
            await get_explorer().search_router_logs(search_term, max_results),
            indent=2,
            ensure_ascii=False,
        )
    ...
    async def diagnose_router_connectivity() -> str:
        ...
        return json.dumps(
            await get_explorer().diagnose_router_connectivity(),
            indent=2,
            ensure_ascii=False,
        )
    ...
    async def get_dhcp_static_leases() -> str:
        """Fetch static DHCP reservations."""
        return json.dumps(
            await get_explorer().get_dhcp_static_leases(), indent=2, ensure_ascii=False
        )
    ...
    async def search_dhcp_logs(search_term: str, hours_back: int = 24) -> str:
        ...
        return json.dumps(
            await get_explorer().search_dhcp_logs(search_term, hours_back),
            indent=2,
            ensure_ascii=False,
        )
    ...
    async def get_device_dhcp_details(
        ...
        return json.dumps(
            await get_explorer().get_device_dhcp_details(mac_address, ip_address),
            indent=2,
            ensure_ascii=False,
        )

    print(
        "[openwrt_explorer] Tools registered (Python-side filtering, enhanced compatibility)"
    )
