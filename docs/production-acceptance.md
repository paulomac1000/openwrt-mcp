...
verification: Run the five lab-marked tests with OPENWRT_LAB_RUN=1 and retain the JUnit evidence.
review_triggers:
...
Set the normal runtime variables plus two laboratory variables:
...
A production-verification record requires **5 passed, 0 failed, 0 skipped** from this file. The tests prove:

1. a deliberately wrong host key prevents creation of a usable command session;
2. the official MCP client can list the 19 closed-schema tools and read real router information;
3. task cancellation invalidates the SSH session, permits a fresh connection, and leaves no matching `ping` process in `ps`;
4. an AsyncSSH command timeout has the same cleanup property and returns exit status 124;
5. the invocation kernel enforces the declared response-byte limit on a real OpenWRT response.
